...
export function DownloadsPage() {
  ...
  return <div className="page downloads-page">
    ...
    })}</div>}{results.length > 0 && nextPage && <div className="catalog-load-more"><button className="secondary-button" disabled={loadingMore} onClick={() => runSearch(nextPage, false)}>{loadingMore ? <><LoaderCircle className="spin"/>Loading more…</> : 'Load more'}</button></div>}</section>
