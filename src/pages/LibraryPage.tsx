...
export function LibraryPage() {
  ...
  return <div className="page"><header className="page-header"><div><span className="eyebrow">{genre ? 'Genre' : 'All books'}</span><h1>{genre?.name || 'Library'}</h1><p>{shown.length} titles</p></div><div className="filter-bar"><SlidersHorizontal size={17}/><select value={format} onChange={(e) => setFormat(e.target.value)}><option value="">All formats</option><option value="epub">EPUB</option><option value="pdf">PDF</option><option value="mobi">MOBI</option></select><select value={sort} onChange={(e) => setSort(e.target.value)}><option value="title">Title</option><option value="author">Author</option><option value="recent">Recently added</option></select></div></header><div className="book-grid">{shown.map((book) => <BookCard key={book.id} book={book}/>)}</div></div>;
}
