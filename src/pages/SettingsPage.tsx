...
export function SettingsPage() {
  ...
  const [path, setPath] = useState('D:\\Books\\Books');
  ...
  return <div className="page settings-page">
    ...
    {adding && <div className="modal-backdrop"><div className="modal"><h2>Add library</h2><label>Name<input value={name} onChange={(event) => setName(event.target.value)}/></label><label>Folder<div className="path-input"><input value={path} onChange={(event) => setPath(event.target.value)}/><button onClick={browse}>Browse…</button></div></label><div className="option-list"><label><input type="checkbox" checked readOnly/>Read metadata.opf and EPUB metadata</label><label><input type="checkbox" checked readOnly/>Use cover.jpg or folder.jpg</label><label><input type="checkbox" checked readOnly/>Watch for changes</label></div><div className="modal-actions"><button className="secondary-button" onClick={() => setAdding(false)}>Cancel</button><button className="primary-button" disabled={busy === 'add-library'} onClick={add}>{busy === 'add-library' && <LoaderCircle className="spin"/>}Add & scan</button></div></div></div>}
  </div>;
}
